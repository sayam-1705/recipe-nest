...
const Carousel = () => {
  ...
  // Use the query hooks for data fetching
  ...
  // Use weather-based recipes if available and not empty, otherwise fallback to all recipes
  const recipeData =
  ...
  // Show weather-specific loading only if we're waiting for weather data and don't have fallback data
  const showWeatherLoading = isLoadingWeather && !allRecipes;
  ...
  return (
    <div
      ...
      id="home"
    >
      {/* Weather Information Banner or General Recipe Banner */}
      {(weatherInfo || (!weatherData && allRecipes)) && (
        <div className="bg-gradient-to-r from-blue-500 to-teal-500 text-white py-2 px-4 text-center text-sm relative">
          {weatherInfo ? (
            <>
              <span className="font-medium">
                Weather in {weatherInfo.place}: {weatherInfo.temperature}°C,{" "}
              ...
              <span className="ml-4 opacity-80">
                {weatherData?.recipes && weatherData.recipes.length > 0
                  ...
                  : "Showing all recipes while weather data loads"}
              </span>
            </>
          ) : (
            <span className="font-medium">
              Discover delicious recipes from our collection • Exploring
          ...
          >
            ...
            {isLoadingWeather ? "Getting weather..." : "Get Weather"}
          </button>
        </div>
      )}
        ...
        <div className="relative h-[500px] overflow-hidden rounded-2xl shadow-2xl bg-primary-orange-bg flex items-center justify-center">
          <div className="text-center">
            <p className="text-secondary-green-dark text-xl font-medium mb-2">
              {isLoadingAllRecipes ? "Loading recipes..." : "No recipes found"}
            </p>
